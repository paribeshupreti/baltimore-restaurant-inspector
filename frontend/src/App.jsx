...
export default function RestaurantHealthScores() {
  ...
  useEffect(() => {
    fetch('/data/baltimore_restaurants.json')
      ...
      .catch(err => {
        ...
        setError(`❌ Unable to load restaurant data - ${err.message}`);
        setLoading(false);
      });
  }, []);
  ...
  const handleShare = (restaurant) => {
    const url = `${window.location.origin}${window.location.pathname}?restaurant=${encodeURIComponent(restaurant.name)}`;
    ...
    <div className={`min-h-screen ${t.bg} ${t.text}`}>
      ...
      {showAboutModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setShowAboutModal(false)}>
          <div className={`${t.card} border-2 ${t.border} rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl`} onClick={(e) => e.stopPropagation()}>
      ...
      {showContactModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setShowContactModal(false)}>
          <div className={`${t.card} border-2 ${t.border} rounded-2xl max-w-lg w-full shadow-2xl`} onClick={(e) => e.stopPropagation()}>
            <div className={`p-6 border-b ${t.border}`}>
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold flex items-center gap-3">
